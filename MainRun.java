...
public class MainRun {
    ...
    private static void showGPA() {
    	...
    	if(!checkScoreData()) {
			System.out.println("Chưa có dữ liệu về bảng điểm. Vui lòng nhập trước đã!");
			return;
		}
    	
    	for(int i = 0; i < STUDENTS.length; i++) {
             if (STUDENTS[i] != null) {
            	 System.out.println("Sinh viên id là " + STUDENTS[i].getId() + " có điểm tổng kết: "+ calculateGPA(STUDENTS[i]));
             }
    	}
    	
	}

    private static int calculateGPA(Student student) {
    	...
    	for(int i = 0; i < SCORES.length; i++) {
             if (SCORES[i] != null) {
            	 ...
            	 if(SCORES[i].getStudent().getId() == student.getId()) {
            		
     				ScoreDetails[] scoreDetails = SCORES[i].getScoreDetails();
     				
     				for(int j = 0; j < scoreDetails.length; j++) {
     					if(scoreDetails[j] != null) {
         					gpa += (scoreDetails[j].getSubject().getUnit() * 
         							scoreDetails[j].getScore());
         					totalUnit = scoreDetails[j].getSubject().getUnit();
     					}
     				}
     			}
             }
    	}
    	return (totalUnit == 0)? 0 : gpa/totalUnit;
    }
	/*
	 * Đọc ghi file
	 */   
